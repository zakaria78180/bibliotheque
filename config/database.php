...
class Database {
    ...
    private function createTables() {
        ...
        // Migration : ajouter la colonne mot_de_passe si la table existait déjà sans
        $colonnes = $this->connection->query("PRAGMA table_info(utilisateurs)")->fetchAll();
        $aMotDePasse = false;
        foreach ($colonnes as $colonne) {
            if ($colonne['name'] === 'mot_de_passe') {
                $aMotDePasse = true;
                break;
            }
        }
        ...
        // Table des avis (liée aux livres et aux utilisateurs)
        $this->connection->exec("
            CREATE TABLE IF NOT EXISTS avis (
                ...
                FOREIGN KEY (livre_id) REFERENCES livres(id),
                FOREIGN KEY (utilisateur_id) REFERENCES utilisateurs(id),
                UNIQUE (livre_id, utilisateur_id)
            )
        ");
    ...
    private function insertDemoData() {
        ...
        if ($result['count'] == 0) {
            ...
            // Insérer des utilisateurs de démonstration (mot de passe : "password")
